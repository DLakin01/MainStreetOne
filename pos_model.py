...
with open("y_pos.pkl", "wb") as pkl:
    pickle.dump(y, pkl)

with open("X_pos.pkl", "wb") as pkl:
    pickle.dump(X, pkl)

predictors = [x for x in X.columns]

gmb0 = GradientBoostingClassifier(random_state=42)
model_fit(gmb0, X, y, predictors)

param_test_1 = {"n_estimators": range(20,81,10)}
grid_search1 = GridSearchCV(
    GradientBoostingClassifier(
        learning_rate=0.1,
        min_samples_split=(0.01 * len(X)),
        min_samples_leaf=(0.001 * len(X)),
        max_depth=8,
        max_features="sqrt",
        subsample=0.8,
        random_state=42
    ),
    param_grid=param_test_1, scoring="roc_auc", iid=False, cv=5
).fit(X[predictors], y)
